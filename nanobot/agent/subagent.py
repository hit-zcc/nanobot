    ...
    async def _run_subagent(
        ...
        try:
            ...
            result = await self.runner.run(AgentRunSpec(
                ...
                # 前台那条 directive 让模型"发完等用户确认"，后台没有交互对象，
                # 等不到确认；这里必须让它自己把成果落地成最终回复。
                iteration_warning_directive=(
                    "[自动提醒 · 非用户发言] 当前后台任务已经接近工具调用的硬限制，"
                    "请立刻停止调用任何新工具。把已经完成的工作整理成一份结构化的"
                    "交付物，直接作为这次的最终回复输出。"
                    "如果产出了文件或提交，必须在回复里写清楚每一项的具体路径和状态；"
                    "同时明确说明哪些部分已经做完、哪些还没做完以及卡在哪里，"
                    "不要把未完成的部分说成已完成。"
                    "没有人会在之后回复你，这条回复就是最终交付。"
                ),
                error_message=None,
